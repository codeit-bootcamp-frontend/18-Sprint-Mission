...
// 닉네임 유효성 검사
const validateNickname = (name, nickname) => {
  nickname.trim().length === 0 ? createErrorMsg(name, "닉네임을 입력해주세요.") : removeErrorMsg(name);
};
// 비밀번호 검사
...
// 비밀번호 일치 검사
const validatePasswordConfirm = (name, password, origin) => {
  password !== origin ? createErrorMsg(name, "비밀번호가 일치하지 않습니다.") : removeErrorMsg(name);
};
...
const validateForm = (name, value) => {
  if (name === "userEmail") {
    ...
    validateNickname(name, value);
  } else if (name === "password") {
    validatePassword(name, value);
  } else if (name === "passwordConfirm") {
    const passwordInput = document.querySelector(`[name="password"]`);
    validatePasswordConfirm(name, value, passwordInput.value);
  }
};
